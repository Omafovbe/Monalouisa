
/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck 
/*
 * This is a barrel export file for all models and their related types.
 *
 * 🟢 You can import this file directly.
 */
export type * from './models/User'
export type * from './models/Account'
export type * from './models/Session'
export type * from './models/VerificationToken'
export type * from './models/Authenticator'
export type * from './models/Teacher'
export type * from './models/Student'
export type * from './models/Admin'
export type * from './models/Subject'
export type * from './models/TeacherApplication'
export type * from './models/Schedule'
export type * from './models/SubjectsOnTeachers'
export type * from './models/StudentsOnTeachers'
export type * from './models/StudentsOnSubjects'
export type * from './models/Subscription'
export type * from './models/Feedback'
export type * from './commonInputTypes'